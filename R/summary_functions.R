...
summaryH <- function(model) {
    ...
    sumry$fstatistic[1] <- car::linearHypothesis(model,white.adjust="hc1", c(names(model$coefficients[-1])), c(rep(0, times=length(names(model$coefficients[-1])))))[2,4]
    p <- nrow(table)
...
summaryHAC <- function(model) {
    ...
    sumry$coefficients <- table
    sumry$fstatistic[1] <- car::linearHypothesis(model,vcov.=sandwich::vcovHAC(model), c(names(model$coefficients[-1])), c(rep(0, times=length(names(model$coefficients[-1])))))[2,4]
    p <- nrow(table)
    hyp <- cbind(0, diag(p - 1))
